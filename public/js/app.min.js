...
const ChainMapRendererBase=function(t){this.ready=function(){return!this.drawing},this.switchTo=function(){if(!document.getElementById("map-container")){const t=document.createElement("div");t.id="map-container",t.className="radial-map",document.getElementById("chainMap").appendChild(t)}this.container=document.getElementById("map-container"),this.container.style.display=""},this.switchFrom=function(){const t=document.getElementById("map-container");t&&t.parentNode.removeChild(t),this.container=null},this.collapse=function(e,o){o?this.mapData.collapsed.push(e):this.mapData.collapsed=this.mapData.collapsed.filter(t=>t!=e),t.updateCollapsed(this.mapData.collapsed),n(this.mapData.map,this.mapData.lines,this.mapData.collapsed)},this.draw=function(t,e,o){this.drawing=!0,this.mapData={map:t,lines:e,collapsed:o};try{n(t,e,o)}finally{this.drawing=!1}},this.arcFactor=function(t){return 1},this.calcMinArc=function(t){return 1};const e=this,n=function(t,n,s){const r=[],c={};for(var a=0;a<t.rows.length;a++){const e=t.rows[a],o=e.c[0],i=o.v,s=e.c[1].v,d={id:i,children:[],systemID:o.systemID,minArc:0};if(null==s){const t={circles:[{arc:0,nodes:[d]}]};d.map=t,d.circle=0,r.push(t)}else{const t=c[s];if(!t)throw"Parent id "+s+" not on map yet";t.children.push(d),d.parent=t,d.connection=n.filter(function(t){return t[0]==i})[0]||[i,s,[],"?"],d.styles=["connection"].concat(d.connection[2]),d.map=t.map,d.circle=t.circle+1,d.map.circles.length<=d.circle?d.map.circles.push({arc:0,nodes:[d]}):d.map.circles[d.circle].nodes.push(d)}c[i]=d,d.markup=o.f,d.requestedSize=measureNode(d.markup)}for(var d=0;d<r.length;d++){const t=r[d];for(var l=t.circles.length-1;l>=1;l--)for(var p=0;p<t.circles[l].nodes.length;p++){const n=t.circles[l].nodes[p];n.minArc*=e.arcFactor(l);const o=e.calcMinArc(n);(n.minArc<o||s.indexOf(1*n.systemID)>=0)&&(n.minArc=o),n.parent.minArc+=n.minArc,t.circles[l].arc+=n.minArc}}for(d=0;d<r.length;d++){const t=r[d];var m=document.getElementById("map"+d);m||((m=document.createElement("div")).id="map"+d,m.className="map-chain-wrapper"),m.innerHTML='<div class="map-outer-container"><div class="map-inner-container"><canvas class="map-drawing" id="map-canvas-'+d+'"/></div></div>';const n=m.firstChild.firstChild;document.getElementById("map-container").appendChild(m);const o=e.initialRads(t.circles[0].nodes[0].minArc);t.bounds=i(n,0,t.circles[0].nodes,-o,o,s),t.radRange={min:-o,max:o},t.domNode=m,t.innerContainer=n}const h=document.getElementById("chainParent"),u={x:h.scrollLeft,y:h.scrollTop};for(d=0;d<r.length;d++){const t=r[d];t.domNode.style.width=null,t.domNode.style.height=null;const n={w:200+t.bounds.x[1]-t.bounds.x[0],h:100+t.bounds.y[1]-t.bounds.y[0],cx:100-t.bounds.x[0],cy:50-t.bounds.y[0]},i=e.centringOptions;if(i.x&&1==r.length){const t=-38+e.container.offsetWidth;n.w<t&&(n.cx+=.5*(t-n.w),n.w=t)}if(i.y){const t=h.offsetHeight;n.h<t&&(n.cy+=.5*(t-n.h),n.h=t)}i.y&&i.rootNode&&n.h>=2*(t.bounds.y[1]>-t.bounds.y[0]?t.bounds.y[1]:-t.bounds.y[0])&&(n.cy=n.h/2),i.x&&i.rootNode&&n.w>=2*(t.bounds.x[1]>-t.bounds.x[0]?t.bounds.x[1]:-t.bounds.x[0])&&(n.cx=n.w/2),t.domNode.style.width=n.w+"px",t.domNode.style.height=n.h+"px";const s=t.domNode.firstChild;s.style.left=n.cx+"px",s.style.top=n.cy+"px";const c=s.getElementsByTagName("canvas")[0];c.width=1*n.w,c.style.width=n.w+"px",c.height=1*n.h,c.style.height=n.h+"px",c.style.left=-n.cx+"px",c.style.top=-n.cy+"px";const a=c.getContext("2d");a.scale(1,1),a.translate(n.cx,n.cy);for(l=t.bounds.maxCi;l>=1;l--)options.chain.gridlines&&e.drawGridlines(a,l,t.radRange);const p=options.chain.lineWeight||1;for(l=t.bounds.maxCi;l>=1;l--)l>=t.circles.length||(a.save(),options.chain.aura&&o(a,t.circles[l].nodes,function(t){a.save(),a.lineWidth=p;const e=propertyFromCssClass(t.styles,"color");a.shadowBlur=11*p,a.shadowColor=e,a.strokeStyle="black";for(let t=0;t<8;t++)a.stroke();a.restore()}),o(a,t.circles[l].nodes,function(t){a.lineWidth=p*parseInt(propertyFromCssClass(t.styles,"border-width"))||2,a.strokeStyle=propertyFromCssClass(t.styles,"border-top-color"),a.setLineDash({dashed:[5,3]}[propertyFromCssClass(t.styles,"border-top-style")]||[]),a.stroke()}),a.restore())}for(d=r.length;;d++){const t=document.getElementById("map"+d);if(!t)break;t.parentNode.removeChild(t)}h.scrollLeft=u.x,h.scrollTop=u.y};function o(t,n,o){for(var i=0;i<n.length;i++){const s=n[i];s.position&&(t.beginPath(),t.moveTo(s.position.x,s.position.y),e.drawConnection(t,s),o(s))}}function i(t,n,o,s,r,c){const a={x:[0,0],y:[0,0],maxCi:n};if(!o.length)return a;const d=c.indexOf(1*(o[0].parent||{}).systemID)>=0,l=(r-s)/(d?o.length:o.reduce(function(t,e){return t+e.minArc},0));var p=s;function m(t){const e=(d?1:t.minArc)*l;return{dr:e,rad_centre:p+e/2}}var h=e.adjustAlignmentDelta(n,m(o[0]).rad_centre);n+=e.skipLevels(n,o,s,r,d);for(var u=0;u<o.length;u++){const s=o[u],r=document.createRange().createContextualFragment('<div class="node-wrapper">'+s.markup+"</div>");s.domNode=r.firstChild,t.appendChild(s.domNode);const l=1*s.systemID;$(s.domNode).dblclick(()=>e.collapse(l,c.indexOf(l)<0));const y=m(s);if(e.setPosition(s,n,y.rad_centre+h),s.position.x<a.x[0]&&(a.x[0]=s.position.x),s.position.x>a.x[1]&&(a.x[1]=s.position.x),s.position.y<a.y[0]&&(a.y[0]=s.position.y),s.position.y>a.y[1]&&(a.y[1]=s.position.y),s.domNode.style.left=s.position.x+"px",s.domNode.style.top=s.position.y+"px",d)s.domNode.style.display="none";else{const e=n>0&&y.dr>s.minArc*n?y.dr-s.minArc*n:0;if(s.children.length){const o=i(t,n+1,s.children,p+.5*e+h,p-.5*e+h+y.dr,c);o.x[0]<a.x[0]&&(a.x[0]=o.x[0]),o.x[1]>a.x[1]&&(a.x[1]=o.x[1]),o.y[0]<a.y[0]&&(a.y[0]=o.y[0]),o.y[1]>a.y[1]&&(a.y[1]=o.y[1]),o.maxCi>a.maxCi&&(a.maxCi=o.maxCi)}}p+=y.dr}return a}this.initialRads=function(t){return Math.PI},this.skipLevels=function(t,e,n,o,i){return 0},this.setPosition=function(t,e,n){throw"must define setPosition"},this.drawConnection=function(t,e){t.lineTo(e.parent.position.x,e.parent.position.y)},this.drawGridlines=function(t,e,n){},this.adjustAlignmentDelta=function(t,e){return 0},this.centringOptions={x:!0,y:!0,root_node:!0}};function propertyFromCssClass(t,e){Array.isArray(t)&&(t=t.join(" "));var n=document.getElementById("temp-div-"+t);return n||((n=document.createElement("div")).id="temp-div-"+t,n.style.cssText="position:fixed;left:-100px;top:-100px;width:1px;height:1px;",n.className=t+" temp",document.body.appendChild(n)),getComputedStyle(n).getPropertyValue(e)}function measureNode(t){var e=document.getElementById("temp-div-node-width");return e||((e=document.createElement("div")).id="temp-div-node-width",e.style.cssText="position:fixed;left:-100px;top:-100px;",e.className="node-wrapper",document.body.appendChild(e)),e.innerHTML=t,e.firstChild.getBoundingClientRect()}
